    ...
    if (_fullURL.includes('Search.aspx')) {
      ...
      getListItems()
        .then(function (response) {
          ...
          // create buffer to remove the duplicate document titles from search
          var temsBuffer = items;

          //Traverse through the buffer item and remove duplicate resources

          for (i = 1; i < temsBuffer.length; i++) {
            if (temsBuffer[i].Title == temsBuffer[i - 1].Title) {
              temsBuffer.splice(i, 1);
            }
          }

          //For loop to traverse through the buffer which is free from duplicate document/s.
          for (i = 0; i < temsBuffer.length; i++) {

            var itemBuffer = temsBuffer[i];

            var _title = itemBuffer.Title;

            _title = _title.toLowerCase();
            _SearchCriteria = _searchKey.toLowerCase();

            if (_title.includes(_SearchCriteria) || itemBuffer.Tag_01.includes(_SearchCriteria) || itemBuffer.Tag_02.includes(_SearchCriteria) || itemBuffer.Tag_03.includes(_SearchCriteria)
              || itemBuffer.Tag_04.includes(_SearchCriteria) || itemBuffer.Tag_05.includes(_SearchCriteria) || itemBuffer.Tag_06.includes(_SearchCriteria)
              || itemBuffer.Tag_07.includes(_SearchCriteria) || itemBuffer.Tag_08.includes(_SearchCriteria) || itemBuffer.Tag_09.includes(_SearchCriteria)
              || itemBuffer.Tag_10.includes(_SearchCriteria) || itemBuffer.Tag_11.includes(_SearchCriteria) || itemBuffer.Tag_12.includes(_SearchCriteria)
              || itemBuffer.Tag_13.includes(_SearchCriteria) || itemBuffer.Tag_14.includes(_SearchCriteria) || itemBuffer.Tag_15.includes(_SearchCriteria)) {

              var _ResourceURL = itemBuffer.ResourceURL;

              // check the resource is ready, if not avoid inserting resource url, instead insert 'coming soon!' after resource title
              if (itemBuffer.Resource_Ready === "Yes") {
                //create row for SearchDataTable with clickable link to access the resource
                mySearchHtml += `
                            <tr>                            
                              <td style="text-align:left;"><a class="document" href="${_ResourceURL}" target="_blank">${itemBuffer.Title}</a></td>                      
                            </tr>
                                        `;
              } else {
                ...
                mySearchHtml += `
                            <tr>                            
                              <td style="text-align:left;"><a class="document" href="javascript: void(0)">${itemBuffer.Title} - <span style="font-weight:bold; color:black;">coming soon!</span></a></td>                      
                            </tr>
                                        `;
              }
...
function searchResourceWithSearchKey(SearchCriteria) {
  ...
  getListItems()
    .then(function (response) {
      ...
      //Create buffer items to remove duplicate titles
      var itemsBuffer = items;


      //Traverse through the buffer items and remove duplicate items
      for (i = 1; i < itemsBuffer.length; i++) {
        if (itemsBuffer[i].Title == itemsBuffer[i - 1].Title) {
          itemsBuffer.splice(i, 1);
        }
      }

      //console.log("itemsBuffer: ", itemsBuffer);
      ...
      for (i = 0; i < itemsBuffer.length; i++) {

        var itemBuffer = itemsBuffer[i];
